(() => {
  ...
  async function renderSettings() {
    ...
    viewRoot.innerHTML = `
      <section class="settings-grid">
        ...
        <article class="setting-card"><div class="eyebrow">PAYMENTS</div><h3>PayPal + wallets</h3><p>Customer checkout uses PayPal. Google Pay and Apple Pay are integrated through PayPal and only render when the merchant account, destination, browser and device are eligible.</p><div class="status-stack"><span>PayPal <b>${config.payments.paypal?'READY':'NOT CONFIGURED'}</b></span><span>Google Pay <b>${config.payments.paypal?'ELIGIBILITY CHECKED AT CHECKOUT':'NOT CONFIGURED'}</b></span><span>Apple Pay <b>${config.payments.paypal?'DOMAIN + ELIGIBILITY REQUIRED':'NOT CONFIGURED'}</b></span><span>Bank / EFT <b>${config.payments.bankTransfer?'READY':'NOT CONFIGURED'}</b></span></div></article>
        ...
        <article class="setting-card"><div class="eyebrow">OWNER PAYOUTS</div><h3>Money to your business</h3><p>SANDMAN never stores raw payout card or bank numbers. Customer payments settle through PayPal; manage your business withdrawal and bank details inside PayPal.</p><div class="button-stack"><a class="btn" href="https://www.paypal.com/businessmanage/money" target="_blank" rel="noreferrer">PayPal business money ↗</a></div></article>
        <article class="setting-card"><div class="eyebrow">MARKETPLACE</div><h3>Seller checkout paused</h3><p>Direct and dropship checkout is PayPal-powered. Marketplace checkout is intentionally disabled until SANDMAN has an approved PayPal-compatible multiparty payout flow; this avoids taking seller payments without a safe payout path.</p><div class="code-block">Historical seller payouts: ${esc(money(config.marketplace.sellerPayoutCents))}\nHistorical SANDMAN fees: ${esc(money(config.marketplace.platformFeeCents))}\nHistorical payout records: ${esc(config.marketplace.payoutCount)}</div></article>
  ...
  document.addEventListener('click', async event => {
    ...
    try {
      ...
      else if (action === 'commerce-add-region') { const country=(prompt('Country code (ZA, US, GB):')||'').trim().toUpperCase();const currency=(prompt('Settlement currency (usually USD):','USD')||'USD').trim().toUpperCase();const locale=(prompt('Locale:','en-ZA')||'en-ZA').trim();const payments=(prompt('Payment providers, comma-separated:','PAYPAL')||'PAYPAL').split(',').map(x=>x.trim().toUpperCase()).filter(Boolean);if(country){await api('/api/admin/commerce/regions',{method:'POST',body:JSON.stringify({country,currency,locale,shippingAllowed:true,taxRequired:true,dutiesRequired:false,paymentMethods:payments})});await renderCommerce();} }
